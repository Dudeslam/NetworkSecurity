...
print("Result of malicious message")
print('Result: ', res2)
...
print("First cookie")
print(firstCook)
print("Request with first cookie")
r3 = requests.get(url + 'quote/', cookies=firstCook)
print("URL response is:%s"%r3.text)
...
print("Malicious Message using json Dumps")
j = json.dumps({'msg': evilMsg.encode('ascii').hex(), 'signature': hex(sigEvil_int)[2:]})
cookay = dict({'msg': evilMsg.encode('ascii').hex(), 'signature': hex(sigEvil_int)[2:]})
# cookay = dict({'msg': base64.b64encode( evilMsg.encode()), 'signature': hex(sigEvil_int)[2:]})
...
# customHeader = {'cookie' : str(r1.cookies)}
customHeader = {'cookie' : j}

# print('request with dict')
# r2 = requests.post(url+'quote/', cookies=firstCook)
# r2 = requests.post(url+'quote/', cookies=r1.cookies, headers=customHeader)
# r2 = requests.post(url+'quote/', cookies=cookay)
# print("URL response is:%s"%r2.text)
# r2 = requests.session()


CookObj=cookiejar_from_dict(cookay)

# CookObj = CookieJar()
# CookObj.make_cookies(r1,r2)
# CookObj.set_cookie(cookay)
print("Cookie from helper function")
print(CookObj)


# print('with session')
# r2 = requests.session()
# cooks = requests.cookies.create_cookie(domain=url, name='grade', value=cookay)
# r2.cookies.set_cookie(cooks)
# print(r2.cookies)


print("CookieJar")
# r3 = requests.get(url + 'quote/', cookies=cookay)
r3 = requests.get(url + 'quote/', cookies=CookObj)
print("URL response is:%s"%r3.text)

# r3 = r2.post(url + 'quote/', cookies=r2.cookies, headers=customHeader)
# print("URL response is:%s"%r3.text)
